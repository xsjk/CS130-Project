...
bool
thread_is_donated(void) 
{
  struct thread *cur = thread_current ();
  return cur->priority != cur->true_priority;
}

/* Directly updates the running priority of the current thread to NEW_PRIORITY,
   yields if necessary. 
*/
void 
thread_set_running_priority (int new_priority) 
{
  struct thread *cur = thread_current ();
...
struct list* 
thread_get_elem_holder(struct thread *t)
{
  return list_of(&t->elem);
}
...
update_recent_cpu(void) 
{
  for (struct list_elem* it = list_begin(&all_list); it != list_end(&all_list); it = list_next(it)) {
    ...
    if (t != idle_thread) {
      ...
      t->priority = fp_to_int_round((fp_sub(fp_create(PRI_MAX), fp_div(t->recent_cpu, fp_create(4))), fp_mul(fp_create(t->nice), fp_create(2))));
    }
  }
}
